...
The model uses a UNet architecture with encoder-decoder structure:
...
┌─────────────────────────────────────┐
│   Encoder Block 1                   │
│   Conv2D(base_filters) → BN → ReLU  │
│   Conv2D(base_filters) → BN → ReLU  │ ← Skip Connection
└─────────────────────────────────────┘
    │
    ▼ MaxPool2D
┌─────────────────────────────────────┐
│   Encoder Block 2                   │
│   Conv2D(base_filters×2) → BN → ReLU│
│   Conv2D(base_filters×2) → BN → ReLU│ ← Skip Connection
└─────────────────────────────────────┘
    │
    ▼ MaxPool2D
┌─────────────────────────────────────┐
│   Bottleneck                        │
│   Conv2D(base_filters×4) → BN → ReLU│
│   Conv2D(base_filters×4) → BN → ReLU│
└─────────────────────────────────────┘
    │
    ▼ Conv2DTranspose (up 2×)
┌─────────────────────────────────────┐
│   Decoder Block 1                   │
...
**Key Features:**
- **Encoder**: 2 downsampling blocks with max pooling
- **Bottleneck**: Single block with 4× filters
- **Decoder**: 2 upsampling blocks with transpose convolutions
- **Skip Connections**: Concatenate encoder features with decoder (U-Net characteristic)
- **Output**: Single channel logits (no sigmoid - applied during training/inference)
- **Base Filters**: Configurable (default: 32 in exp_000, 64 in exp_002)

### Architecture Details

**Encoder Path:**
- Block 1: `base_filters` channels (e.g., 32)
- Block 2: `base_filters × 2` channels (e.g., 64)
- Each block: 2× (Conv2D → BatchNorm → ReLU)

**Bottleneck:**
- `base_filters × 4` channels (e.g., 128)
- Same structure as encoder blocks

**Decoder Path:**
- Block 1: Upsample → Concatenate with Encoder Block 2 → `base_filters × 2` channels
- Block 2: Upsample → Concatenate with Encoder Block 1 → `base_filters` channels

**Loss Function:**
- Binary Cross-Entropy (from_logits=True) for binary segmentation
- Combined with Dice Loss during training
...
```bash
git clone https://github.com/TuanMinhajSeedin/Medical-Image-Segmentation.git
